...
def tokenize_line(content, line_no, source_lines, filename):
    ...
    while i < len(content):
        ...
        if c == " " or c == "\t":
            ...
            elif word in ("fn", "emit", "return", "if", "else", "for", "in", "to",
...
class Parser:
    ...
    def parse_block_stmt(self, block_indent):
        ...
        if toks[0] == ("KEYWORD", "emit"):
            ...
            return {"kind": "emit", "value": value, "line_no": line_no}
        # name = value
        if toks[0][0] == "IDENT" and len(toks) >= 2 and toks[1] == ("SYM", "="):
            name = toks[0][1]
            return self.parse_assignment(line_no, indent, toks, name)
        self.error(line_no, "Inside a function, Moss expected either an 'emit' or a variable assignment (name = value).")
...
def cmd_run(path):
    try:
        ...
        print(e.format(), file=sys.stderr)
        sys.exit(1)
    (RUNTIME_DIR / "src" / "main.rs").write_text(rust)
    r = subprocess.run(
...
def cmd_build(path):
    ...
    if r.returncode != 0:
        sys.exit(r.returncode)
    src_bin = RUNTIME_DIR / "target" / "release" / "moss_run"
    out_name = Path(path).stem
